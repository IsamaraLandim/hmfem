template<class T, class S> class generic_operator
{
public:
...
#endif
/*
template<class T, class S> class entry
{
public:
	T arow;
	T acol;
...
{
private:
	const vector<entry<T, S> > &_aent;
public:
	hilbert_struct_operator(const vector<entry<T, S> > &_aent)
		: _aent(_aent)
	{
	}
	void operator()(const vector<S> &_u, vector<S> &_v) const
	{
#if 1
		const entry<T, S> *__restrict aent = &_aent[0];
		const S *__restrict u = &_u[0];
		S *__restrict v = &_v[0];
...
#else
		int dsize = (int)_v.size();
		for(int i = 0; i < dsize; i++) _v[i] = S(0.0);
		int size = (int)_aent.size();
		for(int i = 0; i < size; i++)
		{
			T p = _aent[i].arow;
...
private:
	...
	const vector<S> &_adia;
public:
	hilbert_struct_symmetric_operator(const vector<entry<T, S> > &_aent, const vector<S> &_adia)
		: _aent(_aent), _adia(_adia)
	{
	}
	void operator()(const vector<S> &_u, vector<S> &_v) const
	{
#if 1
		const entry<T, S> *__restrict aent = &_aent[0];
		const S *__restrict adia = &_adia[0];
		const S *__restrict u = &_u[0];
		S *__restrict v = &_v[0];
